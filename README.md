...
ipp-usb is analyzed by CodeQL (`.github/workflows/codeql.yml`). Because ipp-usb is cgo code, the workflow uses `build-mode: manual` and compiles the daemon itself so CodeQL traces the real build.
...
3. **Custom Workflow Dependencies**:
   - Our custom workflow is designed to install specific project dependencies and perform a manual build before the analysis. This ensures that CodeQL has a complete build graph for the cgo/Go sources in this repository.
